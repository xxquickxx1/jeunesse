      ...
      if(confirm("This will remove the content section '" + tabName + "' immediately even if you don't save this page, are you sure?")) {
        ...
        if($('#page_parts_attributes_' + tabId + '_id').length > 0) {
          ...
          .then(
              function(data,status){
                if (($flash_container = $('#flash_container')).length > 0) {
                  $flash_container.html($('<div class="flash" id="flash">').text(data).append('<a href="#" class="close_icon" id="flash_close"></a>')
                    .addClass((status=='success')? 'flash_notice' : 'flash_error'));
                  $('#flash').css({'width': 'auto', 'visibility': null}).show();
                }
          });
        };
...
(function() {
  this.init_interface = function() {
    ...
    $(".form-actions .form-actions-left input:submit#submit_button").click(function(e) {
      return $("<img src='/assets/refinery/ajax-loader-b9f6dd5edd1222b6fddbc439d64797ef90c669043cb1996a6598f1966563656a.gif' width='16' height='16' class='save-loader' />").appendTo($(this).parent());
    });
